...
Record *testRecord2(Schema *schema, int StudId, char *StudName, int StudAge,float StudMarks,bool Results);
Schema *testSchema2(void);
Record *fromTestRecord2(Schema *schema, TestRecord2 in);
...
{
  Schema *result;
  char *names[] = { "StudId", "StudName", "StudAge","StudMarks","Results" };
...
Record *
fromTestRecord2 (Schema *schema, TestRecord2 in)
{
  return testRecord2(schema, in.StudId, in.StudName, in.StudAge,in.StudMarks,in.Results);
}

Record *
testRecord2(Schema *schema, int StudId, char *StudName, int StudAge,float StudMarks,bool Results)
{
  Record *result;
  Value *value;
  ...
  MAKE_VALUE(value, DT_INT, StudId);
  TEST_CHECK(setAttr(result, schema, 0, value));
  freeVal(value);

  MAKE_STRING_VALUE(value, StudName);
  TEST_CHECK(setAttr(result, schema, 1, value));
  freeVal(value);

  
MAKE_VALUE(value, DT_INT, StudAge);
  TEST_CHECK(setAttr(result, schema, 2, value));
  freeVal(value);

 
MAKE_VALUE(value, DT_FLOAT, StudMarks);
...
testInsertRecords(void)
{
  RM_TableData *table = (RM_TableData *) malloc(sizeof(RM_TableData));
  TestRecord2 inserts[] = { 
    {1, "aaaaaaaaaa", 19,66.6,TRUE}, 
    {2, "bbbbbbbbbb", 20,78.0,TRUE},
    {3, "cccccccccc", 25,34.3,FALSE},
    {4, "dddddddddd", 23,59.8,TRUE},
    {5, "eeeeeeeeee", 18,88.7,TRUE},
    {6, "ffffffffff", 19,25.1,FALSE},
    {7, "gggggggggg", 22,99.2,TRUE},
    {8, "hhhhhhhhhh", 26,72.5,TRUE},
    {9, "iiiiiiiiii", 29,22.6,FALSE},
    {10, "jjjjjjjjj", 19,92.4,TRUE},
  };
  TestRecord2 realInserts[10000];
  TestRecord2 updates[] = {
    {5432, "mmmmmmmmmm", 21,42.3,TRUE}
  };
  int numInserts = 10000, i;
  int randomRec = 5432;
  Record *r;
  RID *rids;
  Schema *schema;
  testName = "test creating a new table and inserting 10000 records then updating record from rids[5432]";
  schema = testSchema2();
  rids = (RID *) malloc(sizeof(RID) * numInserts);
  ...
  for(i = 0; i < numInserts; i++)
    {
      realInserts[i] = inserts[i%10];
      realInserts[i].StudId = i;
      r = fromTestRecord2(schema, realInserts[i]);
      TEST_CHECK(insertRecord(table,r)); 
      rids[i] = r->id;
    }
  TEST_CHECK(closeTable(table));
  ...
  for(i = 0; i < numInserts; i++)
    {
      RID rid = rids[i];
      TEST_CHECK(getRecord(table, rid, r));
      ASSERT_EQUALS_RECORDS(fromTestRecord2(schema, realInserts[i]), r, schema, "compare records");
    }
  
  r = fromTestRecord2(schema, updates[0]);
  r->id = rids[randomRec];
  TEST_CHECK(updateRecord(table,r));
  TEST_CHECK(getRecord(table, rids[randomRec], r)); 
  ASSERT_EQUALS_RECORDS(fromTestRecord2(schema, updates[0]), r, schema, "compare records");
